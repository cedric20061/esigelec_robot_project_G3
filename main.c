...
// choice between HOMOLOGATION or CHOREOGRAPHIE
typedef enum {
  HOMOLOGATION,
  CHOREOGRAPHY,
} Event;
  ...
  if ((TA0CTL & TAIFG == TAIFG)) {
    ...
    if (robot_is_moving) {
      ...
      if (delta_left > delta_right) {
        ...
        TA1CCR1 = PERCENT_CONTROL(
            clamp_percentage(VALUE_TO_PERCENT(TA1CCR1) -
                             SPEED_CORRECTION_GAIN * speed_error_ticks));
        TA1CCR2 = PERCENT_CONTROL(
            clamp_percentage(VALUE_TO_PERCENT(TA1CCR2) +
                             SPEED_CORRECTION_GAIN * speed_error_ticks));
      } else {
      ...
      distance_traveled_mm +=
          ((long)delta_avg_ticks * WHEEL_PERIMETER_MM) / ENCODER_SLOTS_PER_REV;
    }
    choice_event();
     
    // if (distance_sensor_reading < STOP_DISTANCE_THRESHOLD &&
    //     choice_event(HOMOLOGATION)) {
    //   elapsed_seconds++;
    // }
    // if (distance_sensor_reading < STOP_DISTANCE_THRESHOLD &&
    //     choice_event(CHOREOGRAPHY)) {
    //   elapsed_seconds++;
    // } else {
    //   elapsed_seconds++;
    // }
